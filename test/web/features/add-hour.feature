Feature: User can add hours to an existing task
  ...
  Scenario: Get hours worked in a task
    ...
    And there are exactly 0 hours worked in the task

  Scenario: I can register hours worked for a task
    Given a task
    ...
    And there are exactly 8 hours worked in the task
  
   Scenario: Many users can register hours worked for the same task
    Given a task
      |name        |description      |type      |status      |begin_date |assigned_worker_id |
      |Name Test   |Description Test |Type test |Status Test |2018-11-01 |1                  |
    When "I" register hours with the quantity 8 and date of "today"
    When "Someone else" register hours with the quantity 8 and date of "today"
    Then there are exactly 8 hours worked in the task by "me"
    And there are exactly 8 hours worked in the task by "someone else"
    And there are exactly 16 hours worked in the task
